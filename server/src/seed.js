...
const MONGODB_URI =
  process.env.MONGODB_URI || "mongodb://127.0.0.1:27017/cap-predictor";

// Path to the existing cutoffs.json
const CUTOFFS_JSON_PATH = path.resolve(
  import.meta.dirname,
  "../../../cap-predictor/public/data/cutoffs.json",
);
...
async function seed() {
  ...
  console.log(
    `📊 Found: ${data.meta.totalColleges} colleges, ${data.meta.totalBranches} branches, ${data.meta.totalRecords} cutoff records`,
  );
  ...
  console.log(
    `📥 Inserting ${colleges.length} colleges in batches of ${BATCH_SIZE}...`,
  );
  ...
  for (let i = 0; i < colleges.length; i += BATCH_SIZE) {
    ...
    process.stdout.write(
      `\r   Progress: ${inserted}/${colleges.length} (${progress}%)`,
    );
  }
