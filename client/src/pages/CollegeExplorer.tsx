...
import { Building2, MapPin, Search, GraduationCap, ChevronRight, Filter } from 'lucide-react';
...
export function CollegeExplorer() {
  ...
  const filteredColleges = useMemo(() => {
    ...
    return data.colleges.filter(c => {
      ...
      if (searchLower && !c.collegeName.toLowerCase().includes(searchLower) && !c.collegeCode.toString().includes(searchLower)) {
        return false;
      }
      ...
      if (typeFilter !== 'All') {
        ...
        if (typeFilter === 'Govt' && !nameLower.includes('government')) return false;
        if (typeFilter === 'Autonomous' && !nameLower.includes('autonomous')) return false;
        // Approximation for Private (not govt)
        if (typeFilter === 'Private' && nameLower.includes('government')) return false;
      }
  ...
  }, [data, searchTerm, typeFilter, districtFilter]);

  if (loading) {
    return <div className="flex-1 flex items-center justify-center text-slate-400">Loading colleges...</div>;
  }
  if (error || !data) {
    return <div className="flex-1 flex items-center justify-center text-red-400">Error loading data.</div>;
  }
  ...
  return (
    <div className="flex-1 w-full bg-slate-900 pb-20">
      ...
      {/* Header */}
      <div className="bg-slate-800/50 border-b border-white/5 py-12">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <h1 className="text-3xl md:text-5xl font-heading font-extrabold text-white tracking-tight mb-4">
            College Explorer
          </h1>
          <p className="text-slate-400 max-w-2xl mx-auto">
            Browse through {data.colleges.length} engineering institutions in Maharashtra. Discover top colleges and analyze cutoff trends.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-8 flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-64 shrink-0 space-y-6">
          <div className="glass-panel p-5 rounded-2xl border border-white/10 sticky top-28">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 block">Institute Type</label>
                  ...
                  {['All', 'Govt', 'Private', 'Autonomous'].map(type => (
                    <button
                      ...
                      className={`text-left px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                        typeFilter === type ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' : 'text-slate-300 hover:bg-white/5 border border-transparent'
                      }`}
                    >
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs font-medium text-slate-400 mb-4">
                      <span className="flex items-center gap-1">
                        <MapPin className="w-3.5 h-3.5 text-emerald-400" /> {college.district}
                      </span>
                      {college.collegeName.toLowerCase().includes('autonomous') && (
                        <span className="flex items-center gap-1">
                          <GraduationCap className="w-3.5 h-3.5 text-purple-400" /> Autonomous
                        </span>
                      )}
                      <span className="flex items-center gap-1">
                        <Building2 className="w-3.5 h-3.5 text-indigo-400" /> {college.branches.length} Branches
                      </span>
                    </div>
                  </div>

                  <div className="mt-4 pt-4 border-t border-white/5 flex items-center justify-between text-blue-400 font-semibold text-sm group-hover:text-blue-300">
