...
import type { SearchFilters, SortMode, PredictionResult } from '../types';
...
export function PredictorPage() {
  ...
  const [activeTab, setActiveTab] = useState<'predictor' | 'reverse' | 'heatmap'>('predictor');
  const [copiedLink, setCopiedLink] = useState(false);
  const [groupBy, setGroupBy] = useState<'chance' | 'district'>('chance');
  ...
  const { shortlist, toggle, isShortlisted, remove, exportAsText } = useShortlist();
  ...
  // Apply search query match and sorting selection
  const filteredAndSortedResults = useMemo(() => {
    const raw = predictionResults;
    const searchLower = searchTerm.trim().toLowerCase();

    const filterFn = (res: PredictionResult) => {
    ...
    const sortFn = (a: PredictionResult, b: PredictionResult) => {
  ...
  }, [predictionResults, searchTerm, sortMode]);
  ...
  const handleHeatmapCellClick = (district: string, branch: string) => {
    setFilters({
      ...filters,
      districts: [district],
      branches: [branch]
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-slate-900">
        <div className="text-center">
          ...
          <p className="font-heading text-slate-200 text-lg font-bold">Loading Admission Cutoffs...</p>
          <p className="text-slate-400 text-xs mt-1">Parsing historical rounds (2023–2025)</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-slate-900">
        <div className="text-center max-w-md p-8 bg-slate-800/50 border border-red-500/20 rounded-xl shadow-lg backdrop-blur-md">
          <p className="font-heading text-red-400 text-xl font-bold mb-2">Failed to load data</p>
  ...
  const totalResultsCount = filteredAndSortedResults.safe.length + filteredAndSortedResults.moderate.length + filteredAndSortedResults.reach.length;
            ...
            <div className="glass-panel rounded-xl border border-white/10 p-4 mb-6 flex flex-col md:flex-row md:items-center gap-4 shadow-[0_4px_30px_rgba(0,0,0,0.1)]">
              ...
              <div className="flex flex-wrap items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  <span className="font-heading text-[10px] font-bold text-slate-400 uppercase tracking-wider">Sort:</span>
                  <select
                    value={sortMode}
            ...
            <div id="results-section" className="scroll-mt-20">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    Estimated allocation likelihoods based on a target score of <span className="mono font-semibold text-white">{filters.percentile}%</span>.
                  </p>
                </div>
              ...
              {totalResultsCount === 0 ? (
                <div className="glass-panel rounded-xl p-12 text-center text-slate-400">
                  <p className="font-heading text-sm font-semibold text-slate-300">No cutoff predictions found</p>
                  <p className="text-xs mt-1 max-w-md mx-auto">
                    Try raising your percentile score slider, switching categories, or removing advanced filters to explore more options.
                  </p>
                </div>
              ) : displayMode === 'table' ? (
                <div className="animate-fadeIn">
                  <ResultTable
                    ...
                    isShortlisted={(collegeCode, choiceCode) => isShortlisted(collegeCode, choiceCode)}
                    onToggleShortlist={(collegeCode, choiceCode, collegeName, courseName, district) => toggle(collegeCode, choiceCode, collegeName, courseName, district)}
                      ...
                      {filteredAndSortedResults.safe.length === 0 ? (
                        ...
                        filteredAndSortedResults.safe.map((res, i) => (
                          <ResultCard
                            ...
                            isShortlisted={isShortlisted(res.college.collegeCode, res.branch.choiceCode)}
                            onToggleShortlist={() => toggle(res.college.collegeCode, res.branch.choiceCode, res.college.collegeName, res.branch.courseName, res.college.district)}
                            isAddedToOption={hasOption(res.branch.choiceCode)}
                            onToggleOption={() => handleToggleOption(res.college.collegeCode, res.branch.choiceCode, res.college.collegeName, res.branch.courseName, res.college.district)}
                            index={i}
                          />
                        ))
                      )}
                ...
                <div className="space-y-8 animate-fadeIn">
                  {resultsByDistrict.map(group => (
                    <div key={group.district} className="glass-panel rounded-xl p-5 shadow-xs">
