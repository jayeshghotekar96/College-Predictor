...
export function useOptionForm() {
  const [optionForm, setOptionFormState] = useLocalStorage(
    "cap-predictor-option-form",
    [],
  );
  ...
  const removeOption = useCallback(
    (choiceCode) => {
      setOptionFormState(
        reRank(optionForm.filter((item) => item.choiceCode !== choiceCode)),
      );
    },
    [optionForm, setOptionFormState],
  );
  ...
  const setOptionForm = useCallback(
    (items) => {
      setOptionFormState(reRank(items));
    },
    [setOptionFormState],
