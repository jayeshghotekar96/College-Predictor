...
export function useUrlState(defaultFilters) {
  ...
  const setUrlState = (newFilters) => {
    ...
    try {
      ...
      if (newFilters.branches && newFilters.branches.length > 0)
        params.set("br", newFilters.branches.join(","));
      if (newFilters.districts && newFilters.districts.length > 0)
        params.set("dist", newFilters.districts.join(","));
