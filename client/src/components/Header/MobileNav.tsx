...
import { Menu, X, Target, Building2, BarChart3, BookOpen, User, ChevronRight, Search } from 'lucide-react';
          ...
          <>
            ...
            >
              ...
              <div className="p-4 border-b border-white/10 flex items-center justify-between">
                <span className="font-heading font-bold text-white text-lg tracking-tight">Menu</span>
                <button
                  onClick={() => setIsOpen(false)}
              ...
              <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-8">
                ...
                <nav className="flex flex-col gap-2">
                  {[
                    { name: 'Predict', path: '/predict', icon: <Target className="w-5 h-5 text-blue-400" /> },
                    { name: 'Colleges', path: '/colleges', icon: <Building2 className="w-5 h-5 text-amber-400" /> },
                    { name: 'Analytics', path: '/analytics', icon: <BarChart3 className="w-5 h-5 text-rose-400" /> },
                    { name: 'Rank Estimator', path: '/analytics/calculator', icon: <BarChart3 className="w-5 h-5 text-rose-400" /> },
                    { name: 'Resources', path: '/resources', icon: <BookOpen className="w-5 h-5 text-indigo-400" /> },
                  ].map(link => (
                    <button
                      key={link.name}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        {link.icon}
                        <span className="font-semibold text-slate-200 group-hover:text-white">{link.name}</span>
                      </div>
                ...
                <div className="flex flex-col gap-3 pb-6">
                  <div className="flex items-center gap-3 p-4 bg-white/5 border border-white/10 rounded-xl mb-2">
                    ...
                    <div className="flex flex-col flex-1 overflow-hidden">
                      ...
                      <span className="text-xs text-slate-400 truncate">Local Storage</span>
                    </div>
                  </div>
