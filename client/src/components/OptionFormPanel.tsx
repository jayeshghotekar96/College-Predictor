...
interface OptionFormPanelProps {
  optionForm: OptionFormItem[];
  userPercentile: number;
  onRemove: (choiceCode: string) => void;
  onMove: (index: number, direction: 'up' | 'down') => void;
  onRankChange: (choiceCode: string, newRank: number) => void;
  onReorderList: (items: OptionFormItem[]) => void;
  exportAsText: () => string;
}
          ...
          <div className="absolute inset-y-0 right-0 max-w-full pl-10 flex">
            <div className="w-screen max-w-lg glass-panel flex flex-col shadow-xl animate-slideLeft">
              ...
              <div className="bg-ink text-paper px-6 py-5 flex items-center justify-between">
                <div>
                  <h3 className="font-heading text-lg font-bold">Build My Option Form</h3>
              ...
              <div className="flex-1 overflow-y-auto p-6">
                <p className="text-[11px] text-ink-muted mb-4 leading-normal">
                  💡 Drag rows by their handles or use the up/down arrows to reorder your selections. Direct-input rank changes are also supported.
                </p>
                ...
                {optionForm.length === 0 ? (
                  <div className="h-2/3 flex flex-col items-center justify-center text-center text-ink-muted">
                    ...
                    <p className="font-heading text-sm font-semibold text-ink-light">Option form list is empty</p>
                    <p className="text-xs mt-1">Add seats from the predictor results to build your preference form.</p>
                  </div>
                ) : (
                  <div className="flex flex-col gap-3 print-section">
                    ...
                    {optionForm.map((item, idx) => {
                      ...
                      let latestCutoff = 0;
                      if (data) {
                        const college = data.colleges.find(c => c.collegeCode === item.collegeCode);
                        if (college) {
                          const branch = college.branches.find(b => b.choiceCode === item.choiceCode);
                          if (branch) {
                            const gopensCutoffs = branch.cutoffs.filter(c => c.category === 'GOPENS');
                            const maxYear = gopensCutoffs.length > 0 ? Math.max(...gopensCutoffs.map(c => c.year)) : 0;
                            const cutoffRec = gopensCutoffs.find(c => c.year === maxYear);
                      ...
                      <div
                        ...
                        className={`bg-white rounded-md border border-paper-cool/50 flex items-center shadow-xs transition-shadow ${
                          draggedIndex === idx ? 'border-amber opacity-40 shadow-md' : 'hover:shadow-sm'
                        }`}
                      >
                        {/* Drag Handle */}
                        <div className="cursor-grab active:cursor-grabbing p-3 text-ink-muted/50 hover:text-ink transition-colors">
                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
                            <path fillRule="evenodd" d="M3 6a.75.75 0 01.75-.75h16.5a.75.75 0 010 1.5H3.75A.75.75 0 013 6zm0 6a.75.75 0 01.75-.75h16.5a.75.75 0 010 1.5H3.75A.75.75 0 013 12zm0 6a.75.75 0 01.75-.75H18a.75.75 0 010 1.5H3.75A.75.75 0 013 18z" clipRule="evenodd" />
                          </svg>
                        </div>

                        {/* Rank Input */}
                        <div className="flex items-center gap-1 pl-1">
                          <span className="text-[10px] font-heading font-bold text-white/50 uppercase">Rank</span>
                          <input
                            type="number"
                            min="1"
                            max={optionForm.length}
                            value={item.rank}
                            onChange={(e) => onRankChange(item.choiceCode, parseInt(e.target.value) || 1)}
                        ...
                        {latestCutoff > 0 && (
                          <div className="hidden sm:flex flex-col items-center justify-center px-3 border-l border-white/10">
                            <span className="text-[8px] uppercase tracking-wider font-bold text-white/50 mb-0.5">Chance</span>
                            <ProbabilityMeter userPercentile={userPercentile} cutoff={latestCutoff} size={36} strokeWidth={3} />
                          </div>
                        )}
              ...
              <div className="bg-ink p-4 border-t border-paper-cool flex items-center justify-between gap-3">
                ...
                >
                  ...
                  <p className="text-[10px] text-white/50 text-center leading-normal">
                    This exports a formatted list of your ranked choice codes. Keep this clipboard list open while entering options into the government portal to avoid errors.
                  </p>
              </div>
