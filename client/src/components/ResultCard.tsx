...
export function ResultCard({
  ...
  const currentColors = chanceColors[chance as keyof typeof chanceColors] || chanceColors.UNLIKELY;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3 border-b border-white/5 pb-2">
        <span className={`text-[10px] font-heading font-extrabold uppercase tracking-wider px-2 py-0.5 rounded-xs ${currentColors.bg} ${currentColors.text} border border-current`}>
          {chance}
        </span>
      ...
      <div className="mb-4">
        ...
        >
          <h3 className="font-heading text-base md:text-lg font-extrabold text-white leading-snug line-clamp-2 group-hover:text-blue-400 group-hover:underline transition-colors flex items-start gap-2">
            ...
            <span>
              {college.collegeName}
              <span className="inline-block opacity-0 group-hover:opacity-100 transition-opacity translate-x-0 group-hover:translate-x-1 duration-200 ml-1">↗</span>
            </span>
          </h3>
        </Link>
        <div className="flex items-center gap-1.5 mt-2 text-xs text-white/50 pl-7">
          <span className="shrink-0 text-sm">📍</span>
          <span className="font-semibold text-white/70">{college.district}</span>
        </div>
      </div>
      ...
      <div className="bg-black/20 rounded-lg p-3 border border-white/5 mb-4 pl-4 md:pl-5 border-l-2 border-l-blue-500/50">
        ...
        <div className="flex items-center gap-2 text-xs text-white/50">
          ...
          <span className="group/choice relative inline-flex items-center gap-1">
            ...
            >
              {copiedChoice ? <Check className="w-3.5 h-3.5 text-safe" /> : <Copy className="w-3.5 h-3.5" />}
            </button>
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 py-3 border-y border-white/10 bg-white/5 -mx-4 md:-mx-5 px-4 md:px-5">
        ...
        <div className="flex items-center justify-center flex-1 border-x border-white/10 px-2 md:px-4 gap-4 md:gap-6">
          <div className="flex flex-col items-center">
            <span className="text-[9px] uppercase tracking-wider font-heading font-bold text-white/50 mb-1 flex items-center gap-1">🎯 Chance</span>
            <ProbabilityMeter userPercentile={userPercentile} cutoff={latestCutoff} size={36} strokeWidth={4} />
          </div>
          <div className="flex flex-col items-center justify-center">
            <span className="text-[9px] uppercase tracking-wider font-heading font-bold text-white/50 mb-2">Last Cutoff</span>
            <span className="text-lg font-extrabold text-white leading-none">{latestCutoff.toFixed(2)}<span className="text-xs font-normal text-white/50">%</span></span>
          </div>
        </div>

        {/* Best Round */}
        <div className="flex flex-col items-end">
          <span className="text-[9px] uppercase tracking-wider font-heading font-bold text-white/50 mb-1 flex items-center gap-1">⭐ Best Round</span>
      ...
      <div className="flex items-center gap-2 mt-4">
        ...
        >
          <Heart className={`w-3.5 h-3.5 ${isShortlisted ? 'fill-reach' : ''}`} />
        ...
        <button
          onClick={(e) => { e.stopPropagation(); onToggleOption(); }}
          className={`flex-1 flex items-center justify-center gap-1.5 py-2 px-2 rounded-md text-xs font-semibold transition-all border ${
            isAddedToOption 
        ...
        >
          <Scale className={`w-3.5 h-3.5 ${isAddedToOption ? 'fill-amber-500/30' : ''}`} />
