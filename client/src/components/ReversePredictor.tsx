...
import type { College, DecodedCategory } from '../types';
import { reversePredict } from '../lib/prediction';
import { TrendChart } from './TrendChart';

interface ReversePredictorProps {
  colleges: College[];
  categories: Record<string, DecodedCategory>;
  defaultCategory: string;
}

export function ReversePredictor({ colleges, categories, defaultCategory }: ReversePredictorProps) {
  ...
  const selectedBranch = useMemo(() => {
    ...
    return selectedCollege.branches.find(b => b.choiceCode === selectedChoiceCode) || null;
  ...
  const reverseResult = useMemo(() => {
    ...
    const hasCategory = selectedBranch.cutoffs.some(c => c.category === selectedCategory);
    ...
    <div className="glass-panel rounded-md p-5 md:p-6 mb-6">
      ...
      <div className="mb-5">
        ...
        <p className="text-xs text-white/50 mt-1">
          Have a target college and branch in mind? Select them below to estimate the required CET percentile needed to secure a seat.
        </p>
      </div>
          ...
          {!selectedCollegeCode && collegeSearch.trim().length > 1 && (
            <div className="absolute left-0 right-0 mt-1 max-h-60 overflow-y-auto bg-black/80 backdrop-blur-md border border-white/20 rounded-sm shadow-lg z-20 p-1 flex flex-col gap-1">
              ...
              {filteredColleges.length === 0 && (
                <div className="px-3 py-2 text-xs text-white/50">No colleges match your search</div>
              )}
            </div>
          )}
        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
              ...
              <div className="lg:col-span-1 ticket-stub glass-panel border border-white/20 p-5 pl-7 relative">
                ...
                <div className="flex flex-col mb-4">
                  <span className="text-[9px] uppercase tracking-wider text-white/50 font-semibold">Recommended Score</span>
                  <div className="mono text-4xl font-extrabold text-white flex items-baseline">
                    {reverseResult.requiredPercentile.toFixed(2)}
                    <span className="text-sm font-normal text-white/50 ml-1">%ile</span>
                  </div>
                </div>

                <div className="flex flex-col gap-2.5">
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-white/50">Historical Data Count:</span>
                    <span className="font-semibold text-white/80">
                      {selectedBranch.cutoffs.filter(c => c.category === selectedCategory).length} records
                    </span>
                  </div>

                  <div className="flex justify-between items-center text-xs">
                    <span className="text-white/50">Confidence Level:</span>
                    <span className={`text-[10px] uppercase font-bold px-2 py-0.5 border rounded-sm ${
                ...
                <div className="mt-4 text-[10px] text-white/50 leading-relaxed">
                  * Estimated percentile represents a score that yields a <strong>SAFE</strong> placement prediction based on recency-weighted cutoff trend movements.
                </div>
              </div>

              {/* Historical Trend Chart */}
              <div className="lg:col-span-2">
                <TrendChart cutoffs={selectedBranch.cutoffs} category={selectedCategory} />
              </div>

            </div>
          ) : (
            <div className="bg-reach/5 p-4 rounded-sm border border-reach/20 text-center text-reach text-xs">
              ⚠️ No historical records found for category <strong>{selectedCategory}</strong> at this specific branch. Try selecting a different category or course branch.
            </div>
          )}
        </div>
      )}
