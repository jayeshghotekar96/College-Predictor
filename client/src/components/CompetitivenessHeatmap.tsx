...
interface CompetitivenessHeatmapProps {
  ...
  allBranches: string[];
  allDistricts: string[];
  onCellClick: (district: string, branch: string) => void;
}

const DEFAULT_DISTRICTS = ['Pune', 'Mumbai', 'Nagpur', 'Nashik', 'Kolhapur', 'Amravati', 'Aurangabad', 'Thane'];
...
export function CompetitivenessHeatmap({
  ...
  const heatmapData = buildHeatmapData(colleges, category, selectedDistricts, selectedBranches);
    ...
    <div className="glass-panel rounded-md p-5 md:p-6 mb-6">
      ...
      <div className="mb-4">
        ...
        <p className="text-xs text-white/50 mt-1">
          Grid cells display the average cutoff percentile for the selected category. Darker cells represent more competitive (higher cutoff) choices. Click a cell to filter options.
        </p>
      </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {allBranches.slice(0, 12).map(branch => {
              ...
              // Short label
              const short = branch.replace('Engineering', 'Engg.').replace('and', '&').slice(0, 18) + (branch.length > 18 ? '..' : '');
              return (
                <button
                  key={branch}
                  onClick={() => {
      ...
      <div className="overflow-x-auto border border-white/10 rounded-sm">
        <table className="min-w-full divide-y divide-white/10">
          <thead className="bg-white/10">
            <tr>
              <th scope="col" className="px-3 py-2 text-left font-heading text-[10px] font-bold text-white/70 uppercase tracking-wider w-28 bg-transparent sticky left-0 z-10 border-r border-white/20 backdrop-blur-md">
                District
              </th>
              {selectedBranches.map(branch => {
                const shortName = branch.replace('Engineering', 'Engg').replace('and', '&');
                return (
                  <th key={branch} scope="col" className="px-3 py-2 text-center font-heading text-[10px] font-bold text-white/50 uppercase tracking-wider min-w-[120px]" title={branch}>
                    {shortName}
                  </th>
                );
              })}
