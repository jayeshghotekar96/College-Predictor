export function getEquivalentCategories(category) {
  // If user picks an Open category, allow State, Home, and Other variants
  if (["GOPENS", "GOPENH", "GOPENO"].includes(category))
    return ["GOPENS", "GOPENH", "GOPENO"];
  if (["LOPENS", "LOPENH", "LOPENO"].includes(category))
    return ["LOPENS", "LOPENH", "LOPENO"];
...
export function getBestMatchingCategory(cutoffs, requestedCategory) {
...
export function getLatestCutoff(cutoffs, category) {
...
export function projectTrend(cutoffs, category) {
  const catCutoffs = cutoffs.filter((c) => c.category === category);
...
export function classifyChance(studentPercentile, category, cutoffs) {
  const catCutoffs = cutoffs.filter((c) => c.category === category);
  ...
  for (const y of years) {
    const yearData = catCutoffs
      .filter((c) => c.year === y)
      .sort((a, b) => a.round - b.round);
    if (yearData.length >= 2) {
      const drop =
        yearData[0].percentile - yearData[yearData.length - 1].percentile;
...
export function reversePredict(_college, branch, category) {
  ...
  const yearsWithData = new Set(
    branch.cutoffs.filter((c) => c.category === category).map((c) => c.year),
  ).size;
  let confidence = "low";
  if (yearsWithData >= 3) {
    confidence = "high";
...
export function getRoundTiming(cutoffs, category) {
  const catCutoffs = cutoffs.filter((c) => c.category === category);
...
export function predictAll(
  ...
  for (const college of colleges) {
    ...
    for (const branch of college.branches) {
      ...
      const latestCutoff =
        getLatestCutoff(branch.cutoffs, activeCategory) || trend.projected;
  ...
  const sortByPercentile = (a, b) => b.latestCutoff - a.latestCutoff;
...
export function buildHeatmapData(
  ...
  for (const college of colleges) {
    if (
      ...
      !targetDistricts.includes(college.district)
    ) {
      continue;
    }
    ...
    for (const branch of college.branches) {
      if (
        ...
        !targetBranches.includes(branch.courseName)
      ) {
        continue;
      }
